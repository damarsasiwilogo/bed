...
('damar', 'pernah gondrong waktu covid', 'Men', 180, false, 40000000, 99.999, DATE('1994-06-12'), NOW()),
('rizky', 'penikmat anime', 'Men', 170, false, 90000000, 99.999, DATE('2001-01-01'), NOW());
...
where name not in ('damar');
select * from students where name like '%ar%';

-- sql limit clause
select name, min(pocketMoney), max(pocketMoney), sum(pocketMoney)
from students group by name having sum(pocketMoney) >12837123
order by min(pocketMoney) desc, name desc limit 2 offset 1;